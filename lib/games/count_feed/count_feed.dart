...
final countFeedGame = GameDef(
  ...
  prompt: (r) => ['item.${r.str('feeder')}', 'counting.ask', 'number.${r.integer('target')}'],
  shortPrompt: (r) => ['number.${r.integer('target')}', 'item.${r.str('item')}'],
...
class _CountFeedState extends State<CountFeed> {
  ...
  void _accept(String data) {
    setState(() {
      ...
      if (remainingSupply > 0 && _tray.length < kMaxInteractiveItems) _tray.add(_nextTag++);
    });
    rc.hints.succeeded();
    ...
    if (_fed == target) {
      rc.audio.playSequence(['number.$_fed', 'counting.enough']).then((_) => rc.complete());
    } else {
    ...
    return LayoutBuilder(builder: (context, box) {
      ...
      final feederSide = DropTarget(
        ...
        child: ValueListenableBuilder<int>(
          ...
          builder: (_, level, child) => GentlePulse(active: level >= 1, radius: 200, child: child!),
      ...
      return wide
          ? Row(children: [
              ...
              Expanded(child: Center(child: SingleChildScrollView(child: tray))),
            ])
          : Column(children: [
              Expanded(child: Center(child: feederSide)),
...
class _Plate extends StatelessWidget {
  const _Plate({required this.total, required this.filled, required this.food, required this.dot});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Palette.shadow, blurRadius: 8, offset: Offset(0, 4))],
      ),
      child: SizedBox(
                    ...
                    child: i < filled
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: Palette.inkSoft.withValues(alpha: 0.5), width: 3),
                            ),
                          ),
                  ),
                ),
