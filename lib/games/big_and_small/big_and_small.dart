...
class _BigAndSmallState extends State<BigAndSmall> with SingleTickerProviderStateMixin {
  ...
  late final Map<String, GlobalKey> _itemKeys = {for (final id in _order) id: GlobalKey()};
  late final List<GlobalKey> _stepKeys = List.generate(_order.length, (_) => GlobalKey());
  final Set<String> _placed = {};
  String? _missed;

  /// Once all are placed they hop one after another, small → big.
  late final AnimationController _parade =
      AnimationController(vsync: this, duration: const Duration(milliseconds: 1700));

  static const _stepColors = [Color(0xFFB9D98F), Color(0xFFF2C66D), Color(0xFFE9A07A)];
  ...
  double _scaleOf(int i) => _order.length < 2 ? 1 : 0.8 + 0.4 * i / (_order.length - 1);
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, box) {
      ...
      final unit = ((stairsH - card * _scaleOf(n - 1) - 36) / n).clamp(18.0, card * 1.1);
      return Column(children: [
        SizedBox(height: trayH, child: Center(child: _trayView(card))),
  ...
  Widget _stairs({required double card, required double stepW, required double unit}) {
            ...
            Row(
              ...
              children: [
                for (var i = 0; i < _order.length; i++)
                  _step(i, card: card, stepW: stepW, unit: unit, pulse: focus == _order[i]),
              ],
            ),
  ...
  Widget _step(int i, {required double card, required double stepW, required double unit, required bool pulse}) {
    final id = _order[i];
    ...
    return SizedBox(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          child: DropTarget(
            ...
            child: IgnorePointer(
              child: SizedBox.square(
                dimension: size,
                child: placed
                    ? _Settle(from: card / size, child: ItemCard(id: id, size: size))
        ...
        IgnorePointer(child: _Block(width: stepW, height: unit * (i + 1), color: _stepColors[i % _stepColors.length])),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: Palette.card.withValues(alpha: 0.8), width: 3),
...
class _SeatPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final r = RRect.fromRectAndRadius((Offset.zero & size).deflate(2), Radius.circular(radius));
    ...
    for (final metric in (Path()..addRRect(r)).computeMetrics()) {
      for (var d = 0.0; d < metric.length; d += 18) {
        canvas.drawPath(metric.extractPath(d, math.min(d + 10, metric.length)), dash);
      }
    }
  }
