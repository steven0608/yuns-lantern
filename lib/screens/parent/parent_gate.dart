...
void openParentArea(BuildContext context) {
  Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const ParentGate()));
}
...
class _ParentGateState extends State<ParentGate> {
  ...
  void _submit() {
    ...
    if (int.tryParse(_entry) == _a * _b) {
      nav.pushReplacement(MaterialPageRoute<void>(builder: (_) => const ParentArea()));
    } else {
      nav.pop();
    }
  }
                ...
                children: [
                  ConstrainedBox(
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      Text(l.gateTitle, style: const TextStyle(fontSize: 20, color: Palette.inkSoft, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 12),
                      Text(
                        l.gateQuestion(_word(l, _a), _word(l, _b)),
                        textAlign: TextAlign.center,
                        style: const TextStyle(fontSize: 26, color: Palette.ink, fontWeight: FontWeight.w700),
                      ),
                      ...
                      Container(
                        ...
                        child: Text(_entry, style: const TextStyle(fontSize: 34, fontWeight: FontWeight.w800, color: Palette.ink)),
                      ),
                    ]),
                  ),
                  Column(mainAxisSize: MainAxisSize.min, children: [
                    for (final row in const [['1', '2', '3'], ['4', '5', '6'], ['7', '8', '9']])
                      Row(mainAxisSize: MainAxisSize.min, children: [for (final d in row) key(d, () => _press(d))]),
